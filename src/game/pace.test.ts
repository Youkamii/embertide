...
describe('페이스 행렬 — 포식 시간의 단조성 계약', () => {
  it('큰 것일수록 오래 걸리고, 내가 클수록 빨라진다 (역전 금지)', () => {
    ...
    for (const myR of [7, 15, 30]) {
      ...
      // 소형 표적(금성·목성)은 ×1000 기저에서 전부 1~5초로 압축되고, 충돌
      // 흡수 물리로 먹이가 공성 중 주변 잔챙이를 먹으며 도로 크는 노이즈까지
      // 있다 (계측: r 12.40→12.54). 자릿수 역전만 막는다.
      expect(ven, `R${myR}: 금성이 목성보다 터무니없이 오래 걸리면 안 된다`)
        .toBeLessThan(jup * 3 + 2)
      // 동률 허용(×1.5+2) — 충돌 흡수 노이즈 + 경로 차(토성은 스파게티 예열
      // 구간, 태양은 dom 잠식)로 초 단위가 흔들린다 (계측: R7 토성 4.30 vs
      // 태양 4.25, R30 토성 2.88 vs 태양 1.32 — 전부 "찰나" 범주).
      // 태양이 자릿수로 빨리 죽는 역전만 잡는다.
      expect(jup, `R${myR}: 목성 ≲ 태양`).toBeLessThan(sun * 1.5 + 2)
      expect(sat, `R${myR}: 토성 ≲ 태양`).toBeLessThan(sun * 1.5 + 2)
    }
