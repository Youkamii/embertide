...
describe('페이스 행렬 — 포식 시간의 단조성 계약', () => {
  it('큰 것일수록 오래 걸리고, 내가 클수록 빨라진다 (역전 금지)', () => {
    // 실측 (2026-07-18, 에딩턴 R^1.6): R7 → 지구 0.6 / 토성 4.5 / 목성 5.0 / 태양 13.4s
    //                                  R30 → 전부 수 초 이하. 역전이 생기면 여기서 잡힌다.
    ...
    // 태양은 작을 땐 공성전, 커지면 순식간 — 성장의 서사
    expect(sunAt[7]!, '작을 때 태양은 공성전(15초+, 실측 20.6s)').toBeGreaterThan(15)
    expect(sunAt[30]!, '커지면 태양이 빨라진다 (실측 20.6→14.3s)').toBeLessThan(sunAt[7]! * 0.75)
