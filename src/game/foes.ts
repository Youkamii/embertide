 ...
 * 두 경로(진형·성흔)가 쓴다. **링(rollRingPos)은 반대편 접기다 — 클램프로
 * 되돌리지 말 것**: 클램프는 guard 원의 플레이어 쪽 호에 뭉쳐 240px 코앞 스폰을
 * 만들었다(적대 리뷰가 기하로 증명). 진형은 bearing 이 정보라 접으면 진형이 깨져
 * 클램프가 맞고, 성흔은 낱개 아이템이라 어느 쪽이든 무해해 싼 쪽을 쓴다.
