 ...
 * 눈금: **실척** — 전 구간 1광년 = 50,000px (Phase 2 에서 로그 압축 폐지).
 * 은하 중심 26,673광년 = 13.3억px, 안드로메다 253만 광년 = 1,268억px.
 * 로그 항행(속도 ∝ 남은 거리)이라 어디든 십수~수십 초 — 스케일은 정직하게,
 * 이동은 게임답게.
 *
...
/** 실척 (Phase 2, 아키텍처 §2-1) — 로그 무릎 폐지, 전 구간 1광년 = 50,000px.
 *  궁수자리 A* 1.33e9px, 안드로메다 1.27e11px. 시뮬은 double 이라 무한소
 *  오차, 렌더는 부동 원점(Phase 1)이 지킨다. 은하 밖 공허가 정직해진다. */
export function pxOf(ly: number): number {
  return ly * LY
}

/** 게임 px → 실거리 광년 — 이제 정확한 역함수다 (마젤란 "300광년" 버그 소멸) */
